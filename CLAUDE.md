...
The SDK is **initialized programmatically** — `GameManager.Start()` calls `AdManager.Initialize(new AdMediationConfig { LevelPlayAppKey, LevelPlayInterstitialAdUnitId, LevelPlayRewardedAdUnitId, GamInterstitialAdUnitId, GamRewardedAdUnitId })` exactly once with the publisher's credentials. No scene GameObject or Inspector wiring is needed. From then on the game calls `AdManager.TryShowInterstitial(onContinue)` and `AdManager.TryShowRewarded(onRewardEarned, onAdClosed)`.

The five `AdMediationConfig` fields are the only things that vary per publisher; everything else is hardcoded in SDK source. Per-format price-floor mappings live as `abstract` property overrides on each subclass:

- `InterstitialMediationController` — `GamPriceFloorKey = "ULFP_2"`, `PriceFloorMarkup = 1.2`, `PriceFloorBuckets = [0.1, 1, 2, …, 10]`.
- `RewardedMediationController` — `GamPriceFloorKey = "ULFP_RWD"`, `PriceFloorMarkup = 1.2`, `PriceFloorBuckets = [1, 5, 10, 15, 20, 25, 30, 40, 50]` (placeholders — coordinate with ad-ops).

Both controllers extend the abstract `MediationAuctionController`, which holds the shared auction state machine, floor-signal math, eCPM cache, and the SDK constants (`AuctionTimeoutSeconds = 6f`, `DefaultLevelPlayEcpm = 1.0`, `LevelPlayEcpmPrefsKeyPrefix = "AdMgr_LP_LastEcpm"`). They run as **fully independent auctions** — separate round counters, separate ad objects, separate cached LevelPlay eCPM (per-format PlayerPrefs key `AdMgr_LP_LastEcpm_<Format>`), separate GAM floors derived from each format's own eCPM history. A `GamBannerController` singleton is also created by `Initialize`.

GameManager triggers interstitials at three points in `AnimateCameraToStartPosition`'s `OnComplete`: on Won (no callback), on Lost/last-bird (no callback), or between birds (callback advances `currentBirdIndex` and animates the next bird in). **The continuation callback pattern matters** — if you bypass `AdManager.TryShowInterstitial(onContinue)` / `TryShowRewarded(...)` and call gameplay advance directly, the game will advance before/regardless of the ad.

If `TryShow*` is called before `AdManager.Initialize`, the SDK logs a warning and invokes the continuation immediately (fail-open). If a leg's ad unit id is empty in the config, that leg is skipped at bootstrap (e.g., `LevelPlayRewardedAdUnitId = ""` disables LP rewarded while interstitial works normally).
